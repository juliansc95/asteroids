...
AngelcodeFont* pokemon100font = NULL;
AngelcodeFont* OldStamper100 = NULL;
...
void Init()
{
	...
	//load a font
	pokemon100font = blit3D->MakeAngelcodeFontFromBinary32("Media\\pokemon100.bin");
	OldStamper100 = blit3D->MakeAngelcodeFontFromBinary32("Media\\OldStamper100.bin");

	//load our small ship life counter
	lifeSprite = blit3D->MakeSprite(0,0,30,27,"Media\\Ship_small.png");

	//load our background image: the arguments are upper-left corner x, y, width to copy, height to copy, and file name.
	backgroundSprite = blit3D->MakeSprite(0, 0, 1920, 1080, "Media\\cheese.png");

	//create a ship
	ship = new Ship;
	//load a sprite off of a spritesheet
	for (int i = 0; i < 4; ++i)
		ship->spriteList.push_back(blit3D->MakeSprite(i * 72, 0, 72, 88, "Media\\shipsF.png"));
	...
	musicID = audioE->PlayEvent("Streaming", mainGameID);


	audioE->PauseEvent("Streaming", mainGameID, musicID);
	audioE->PauseEvent("Thrust", mainGameID, thrustID);

	//load our asteroid graphics
	largeAsteroid = blit3D->MakeSprite(0, 0, 288, 292, "Media\\asteroidR5.png");
	mediumAsteroid = blit3D->MakeSprite(0, 0, 77, 72, "Media\\medium.png");
	smallAsteroid = blit3D->MakeSprite(0, 0, 41, 39, "Media\\small.png");
	...
	titleGraphic = blit3D->MakeSprite(0, 0, 1920, 1080, "Media\\cheese.png");
...
void Update(double seconds)
{
	...
	case GameState::PLAYING:
		...
		audioE->ResumeEvent("Streaming", mainGameID, musicID);
		...
		while (elapsedTime >= timeSlice)
		{
			...
			for (int aIndex = asteroidList.size() - 1; aIndex >= 0; --aIndex)
			{
				for (int sIndex = shotList.size() - 1; sIndex >= 0; --sIndex)
				{
					...
					if (Collide(shotList[sIndex], asteroidList[aIndex]))
					{
						...
						{
							//20 points for medium asteroids
							score += 20;
							//make an explosion here
			...
			for (auto A : asteroidList)
			{
				...
				if (Collide(ship, A))
				{
					//Make a explosion sound when collide
					explosionID = audioE->PlayEvent("Explosion", mainGameID);
					//take away a life
					ship->lives--;
...
void Draw(void)
{
	...
	case GameState::HIGHSCOREENTRY:
	{
		...
		float widthTitle = OldStamper100->WidthText(titleString);
		OldStamper100->BlitText((1920.f / 2) - (widthTitle / 2), 1080.f - (100 + 75), titleString);

		OldStamper100->BlitText(400, 590, "ENTER YOUR INITIALS: ");
		
		for(int i = 0; i <= currentLetter; ++i)
			OldStamper100->BlitText(900 + i * 50, 400 , &initials[i]);

		//draw the score
		std::string scoreString = std::to_string(score);
		pokemon100font->BlitText(1000, 1080 - 5, scoreString);
	}
		break;
	...
	case GameState::HIGHSCORE:
	{
		...
		for (int i = 0; i < hiScoreTable.size(); ++i)
		{
			...
			OldStamper100->BlitText(500, 1080.f - (300 + 120 * i), scoreLine);
		}
	...
	case GameState::TITLESCREEN:
	{
		...
		std::string titleString = "CHEESE BLASTER";
		float widthTitle = OldStamper100->WidthText(titleString);
		OldStamper100->BlitText((1920.f / 2) - (widthTitle / 2), 1080.f / 2, titleString);
	}
		break;
	case GameState::PAUSE:
	{
		...
		float widthText = pokemon100font->WidthText(paused);
		pokemon100font->BlitText(1920.f / 2.f - widthText / 2.f, 1080.f / 2.f, paused);
		//draw the score
		std::string scoreString = std::to_string(score);
		pokemon100font->BlitText(1000, 1080 - 5, scoreString);
	}
	...
	case GameState::GAMEOVER:
	{
		...
		float widthText = pokemon100font->WidthText(gameOver);
		pokemon100font->BlitText(1920.f / 2.f - widthText / 2.f, 1080.f / 2.f, gameOver);
		std::string gameOver2 = "Press G to start a new game";
		float widthText2 = pokemon100font->WidthText(gameOver2);
		pokemon100font->BlitText(1920.f / 2.f - widthText2 / 2.f, 1080.f / 2.f - 100.f, gameOver2);
		//draw the score
		std::string scoreString = std::to_string(score);
		pokemon100font->BlitText(1000, 1080 - 5, scoreString);
	}
